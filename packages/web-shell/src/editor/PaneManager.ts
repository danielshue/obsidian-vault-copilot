...
import type { PaneTreeState, PaneNodeState, PaneLeafState, PaneSplitState } from "../settings/WebShellSettings.js";
import { loadSettings, saveSettings } from "../settings/WebShellSettings.js";
...
export class PaneManager {
	...
	private async restoreNode(state: PaneNodeState): Promise<PaneNode> {
		...
		for (let i = 0; i < splitState.children.length; i++) {
			const child = await this.restoreNode(splitState.children[i]);

			// Apply saved flex size
			if (splitState.sizes[i]) {
				child.element.style.flex = splitState.sizes[i];
			}
		...
		for (let i = 0; i < resizers.length; i++) {
			this.initPaneResizer(resizers[i], splitNode, i);
		}
	...
	private replaceInChildren(node: PaneNode, target: PaneNode, replacement: PaneNode): boolean {
		...
		for (let i = 0; i < node.children.length; i++) {
			...
			if (this.replaceInChildren(node.children[i], target, replacement)) return true;
		}
		return false;
	}
