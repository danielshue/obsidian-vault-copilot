...
import { parseYaml, stringifyYaml } from "@vault-copilot/obsidian-shim/src/utils/parseYaml.js";
...
function renderValueEditor(
	...
	if ((type === "list" || type === "tags") && Array.isArray(value)) {
		...
		dateText.addEventListener("click", () => {
			...
			if (dateMatch) input.value = dateMatch[1];
...
function handleAddProperty(
	...
	const updateSuggestSelection = () => {
		...
		for (let i = 0; i < children.length; i++) {
			children[i].classList.toggle("is-selected", i === suggestIndex);
		}
	...
	keyInput.addEventListener("keydown", (e) => {
		if (suggestEl && suggestItems.length > 0) {
			...
			if (e.key === "Enter" && suggestItems[suggestIndex]) {
				e.preventDefault();
