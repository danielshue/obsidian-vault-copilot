...
import { loadSettings, settingsChanged } from "../settings/WebShellSettings.js";
...
export class WebShellApp {
		...
		try {
			...
			if (viewType === "ws-file-tab-view") {
				...
				editor.setSaveHandler(async (path, content) => {
					...
					await vault.modify(file as import("@vault-copilot/obsidian-shim/src/vault/TFile.js").TFile, content);
				});
	...
	private static async _loadCopilotPlugin(): Promise<any> {
		const attempts: Array<{
			...
			{ label: "source extensionless specifier", load: () => import("../../../../src/main") },
		];
