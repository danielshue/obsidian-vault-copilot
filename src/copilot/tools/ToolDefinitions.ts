...
export const TOOL_DESCRIPTIONS = {
	...
	[TOOL_NAMES.GET_DAILY_NOTE]: "Get today's daily note or a daily note for a specific date. Use this only for daily-note content/metadata lookups, not for general vault task queries.",
	...
	[TOOL_NAMES.LIST_TASKS]: "List tasks from the vault (or from a specific note/folder path). Prefer this for questions like 'what tasks did I create today', 'my open tasks', or other task summaries.",
	[TOOL_NAMES.TASKS_QUERY]: "Run an Obsidian Tasks-style query string (for example: 'not done', 'due before tomorrow', 'limit 20'). Merges Tasks plugin cache results with direct vault parsing so missing cache entries are still included.",
	...
	[TOOL_NAMES.RENDER_MERMAID_DIAGRAM]: "Build and validate a Mermaid fenced markdown block for document generation. Use before create_note/update_note when a document requires a diagram.",
...
/** Parameters for render_mermaid_diagram */
export interface RenderMermaidDiagramParams {
	/** Mermaid diagram type */
	diagramType: MermaidDiagramType;
	/** Mermaid body content (with or without the diagram type on first line) */
	mermaidBody: string;
	/** Optional diagram title for surrounding document context */
	title?: string;
}
...
export const TOOL_JSON_SCHEMAS: Record<string, JsonSchemaObject> = {
	...
	[TOOL_NAMES.REPLACE_NOTE]: {
		...
		properties: {
			path: { type: "string", description: "The path to the note file" },
			content: { type: "string", description: "The new content to replace the entire note with" }
		},
		required: ["path", "content"]
	},
	...
	[TOOL_NAMES.OPEN_WEEKLY_NOTE]: {
		type: "object",
		properties: {
			period: { type: "string", description: "Week expression like 'this week', 'last week', 'W05'" }
		},
		required: []
	},
	...
	[TOOL_NAMES.OPEN_MONTHLY_NOTE]: {
		type: "object",
		properties: {
			period: { type: "string", description: "Month expression like 'this month', 'last month', 'January 2026'" }
		},
		required: []
	},
	...
	[TOOL_NAMES.OPEN_QUARTERLY_NOTE]: {
		type: "object",
		properties: {
			period: { type: "string", description: "Quarter expression like 'this quarter', 'Q1 2026'" }
		},
		required: []
	},
	...
	[TOOL_NAMES.OPEN_YEARLY_NOTE]: {
		type: "object",
		properties: {
			period: { type: "string", description: "Year expression like 'this year', 'last year', '2026'" }
		},
		required: []
	},
	...
	[TOOL_NAMES.GET_TASKS]: {
		type: "object",
		properties: {
			path: { type: "string", description: "Path to the note. If not provided, uses the active note." }
		},
		required: []
	},
	...
	[TOOL_NAMES.CREATE_TASK]: {
		type: "object",
		properties: {
			path: { type: "string", description: "The path to the note where the task should be added" },
			description: { type: "string", description: "The task description text" },
			priority: {
				type: "string",
				enum: ["highest", "high", "medium", "low", "lowest", "none"],
				description: "Priority level (highest, high, medium, low, lowest, none)"
			},
			dueDate: { type: "string", description: "Due date in YYYY-MM-DD format (📅)" },
			scheduledDate: { type: "string", description: "Scheduled date in YYYY-MM-DD format (⏳)" },
			startDate: { type: "string", description: "Start date in YYYY-MM-DD format (🛫)" },
			recurrence: { type: "string", description: "Recurrence rule (e.g., 'every day', 'every week', 'every month on the 1st')" },
			tags: {
				type: "array",
				items: { type: "string" },
	...
	[TOOL_NAMES.MARK_TASKS]: {
		...
		properties: {
			tasks: {
				type: "array",
				items: { type: "string" },
				description: "Array of task description text strings to modify (text after the checkbox)"
			},
			complete: { type: "boolean", description: "true to mark tasks complete ([x]), false to mark incomplete ([ ]). Default: true" },
			path: { type: "string", description: "Path to the note. If not provided, uses the active note." },
			exceptions: {
				type: "array",
				items: { type: "string" },
				description: "Task text strings to exclude from the operation"
			}
		},
		required: ["tasks"]
	},
	...
	[TOOL_NAMES.LIST_TASKS]: {
		type: "object",
		properties: {
			path: { type: "string", description: "Optional note or folder path to scope results. If omitted, searches tasks across the entire vault." },
			...
			dueBefore: { type: "string", description: "Filter tasks due before this date (YYYY-MM-DD, inclusive)" },
			dueAfter: { type: "string", description: "Filter tasks due after this date (YYYY-MM-DD, inclusive)" },
			dueOn: { type: "string", description: "Filter tasks due on this exact date (YYYY-MM-DD)" },
			createdBefore: { type: "string", description: "Filter tasks created before this date (YYYY-MM-DD, inclusive; uses ➕ created date metadata)" },
			createdAfter: { type: "string", description: "Filter tasks created after this date (YYYY-MM-DD, inclusive; uses ➕ created date metadata)" },
			createdOn: { type: "string", description: "Filter tasks created on this exact date (YYYY-MM-DD; uses ➕ created date metadata)" },
			tags: {
				type: "array",
				items: { type: "string" },
				description: "Tags to filter by (without # prefix)"
			},
			query: { type: "string", description: "Search query to filter tasks by description text" },
			limit: { type: "number", description: "Maximum number of results to return" }
		},
		required: []
	},

	[TOOL_NAMES.TASKS_QUERY]: {
		type: "object",
		properties: {
			query: {
				type: "string",
				description: "Obsidian Tasks query text. Supports one instruction per line (for example: 'not done', 'due before 2026-03-01', 'limit 50')."
			},
			path: {
				type: "string",
				description: "Optional note or folder path to scope tasks before applying the query. If omitted, the entire vault is searched."
			},
			limit: {
				type: "number",
				description: "Optional hard cap for returned tasks. Applied after query instructions."
			}
		},
		required: ["query"]
	},

	[TOOL_NAMES.MARK_TASKS_COMPLETE]: {
		type: "object",
		properties: {
			task_list: {
				type: "array",
				items: { type: "string" },
				description: "Array of task description text strings to mark complete"
			},
			exceptions: {
				type: "array",
				items: { type: "string" },
			...
			notePath: { type: "string", description: "Path to the note containing the tasks. If not provided, uses the active note." }
		},
		required: ["task_list"]
	},
	...
	[TOOL_NAMES.SEND_TO_CHAT]: {
		type: "object",
		properties: {
			content: { type: "string", description: "The markdown content to display in chat" },
			title: { type: "string", description: "Optional title to prepend as a heading" }
		},
		required: ["content"]
	},
	...
	[TOOL_NAMES.RENDER_MERMAID_DIAGRAM]: {
		type: "object",
		properties: {
			diagramType: {
				type: "string",
			...
			mermaidBody: {
				type: "string",
				description: "Mermaid body content. Can optionally include the diagram type on the first line."
			},
			title: {
				type: "string",
				description: "Optional diagram title for document context"
			}
		},
		required: ["diagramType", "mermaidBody"]
	},
...
export const PARAM_DESCRIPTIONS = {
	...
	/** Obsidian Tasks query text */
	tasksQuery: "Obsidian Tasks query text with one instruction per line (for example: 'not done', 'due before tomorrow', 'limit 20')",
