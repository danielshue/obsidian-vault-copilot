...
export class ExtensionSubmissionModal extends Modal {
	...
	/**
	 * Welcome screen: Introduction to the submission wizard
	 * 
	 * @param container - Container element
	 * @internal
	 */
	private renderWelcomeStep(container: HTMLElement) {
		...
		new ButtonComponent(buttonContainer)
			...
			.setCta()
			.onClick(() => {
				this.currentStep = 0; // Move to extension selection
				this.renderCurrentStep();
			});
	...
	private async validateCurrentStep(): Promise<boolean> {
		...
		switch (this.currentStep) {
			...
			// Step 2 (Description) and Step 3 (README): Both optional, no validation required
			case 2:
			case 3:
				return true;
		}
