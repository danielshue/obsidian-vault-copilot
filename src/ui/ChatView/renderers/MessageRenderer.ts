...
import type { RenderRegistry } from "../../../api/registries/RenderRegistry";

// TTS stub types for Basic (TTS is Pro-only)
type TTSConfig = { apiKey?: string; baseUrl?: string };
class TTSService {
	constructor(_config: TTSConfig) {}
	async synthesize(_text: string, _options: { voice?: string; format?: string }): Promise<ArrayBuffer | null> {
		return null;
	}
}
...
export class MessageRenderer {
	...
	private renderRegistry: RenderRegistry | null = null;

	/** Pane identifiers used for internal-link open targets. */
	private static readonly LINK_PANE_LEFT = "left" as const;
	private static readonly LINK_PANE_MIDDLE = "middle" as const;
	private static readonly LINK_PANE_RIGHT = "right" as const;
	private static readonly LINK_DEFAULT_PANE_STORAGE_KEY = "vault-copilot:chat-link-default-pane";
	...
	/**
	 * Set the render registry for post-processing extensions.
	 * @param registry - The render registry from the Extension API
	 */
	setRenderRegistry(registry: RenderRegistry): void {
	...
	async renderMarkdownContent(messageEl: HTMLElement, content: string): Promise<void> {
		...
		if (oldContentEl) {
			...
			// Apply registered render extensions (post-processing)
			if (this.renderRegistry) {
				await this.renderRegistry.processAll(newContentEl);
			}
		...
		for (const ref of refs) {
			...
			if (ref.type !== "url" && ref.path) {
				...
				nameEl.addEventListener("click", (e) => {
					e.stopPropagation();
					void this.openInternalLinkInPane(ref.path, this.getDefaultLinkPane());
				});
				nameEl.addEventListener("contextmenu", (e) => {
					e.preventDefault();
					e.stopPropagation();
					this.showInternalLinkPaneMenu(e, ref.path);
				});
	...
	/**
	 * Show a context menu for opening an internal link in a specific pane.
	 *
	 * @param event - Mouse event used to position the menu
	 * @param linkPath - Internal link target path/text
	 */
	private showInternalLinkPaneMenu(event: MouseEvent, linkPath: string): void {
		const menu = new Menu();
		const defaultPane = this.getDefaultLinkPane();

		menu.addItem((item) => {
			item.setTitle("Open in Left Pane");
			if (defaultPane === MessageRenderer.LINK_PANE_LEFT) {
				item.setIcon("check");
			}
			item.onClick(() => {
				this.setDefaultLinkPane(MessageRenderer.LINK_PANE_LEFT);
				void this.openInternalLinkInPane(linkPath, MessageRenderer.LINK_PANE_LEFT);
			});
		});

		menu.addItem((item) => {
			item.setTitle("Open in Middle Pane");
			if (defaultPane === MessageRenderer.LINK_PANE_MIDDLE) {
				item.setIcon("check");
			}
			item.onClick(() => {
				this.setDefaultLinkPane(MessageRenderer.LINK_PANE_MIDDLE);
				void this.openInternalLinkInPane(linkPath, MessageRenderer.LINK_PANE_MIDDLE);
			});
		});

		menu.addItem((item) => {
			item.setTitle("Open in Right Pane");
			if (defaultPane === MessageRenderer.LINK_PANE_RIGHT) {
				item.setIcon("check");
			}
			item.onClick(() => {
				this.setDefaultLinkPane(MessageRenderer.LINK_PANE_RIGHT);
				void this.openInternalLinkInPane(linkPath, MessageRenderer.LINK_PANE_RIGHT);
			});
		});
	...
	/**
	 * Open an internal link in a specific pane when that pane API is available.
	 *
	 * Falls back to default `openLinkText` behavior if pane-specific APIs are
	 * unavailable (for example, in standard Obsidian runtime).
	 *
	 * @param linkPath - Internal link target path/text
	 * @param pane - Target pane identifier
	 */
	private async openInternalLinkInPane(
		linkPath: string,
		pane: typeof MessageRenderer.LINK_PANE_LEFT | typeof MessageRenderer.LINK_PANE_MIDDLE | typeof MessageRenderer.LINK_PANE_RIGHT,
	): Promise<void> {
		type WorkspaceWithPaneTargets = App["workspace"] & {
			getLeftLeaf?: (split: boolean) => unknown;
			getMiddleLeaf?: (split: boolean) => unknown;
			getRightLeaf?: (split: boolean) => unknown;
			revealLeaf?: (leaf: unknown) => void;
		};

		const workspace = this.app.workspace as WorkspaceWithPaneTargets;
		let targetLeaf: unknown = null;

		if (pane === MessageRenderer.LINK_PANE_LEFT) {
			targetLeaf = workspace.getLeftLeaf?.(false) ?? null;
		} else if (pane === MessageRenderer.LINK_PANE_MIDDLE) {
			targetLeaf = workspace.getMiddleLeaf?.(false) ?? null;
		} else {
			targetLeaf = workspace.getRightLeaf?.(false) ?? null;
		}

		if (targetLeaf && typeof workspace.revealLeaf === "function") {
			workspace.revealLeaf(targetLeaf);
			this.app.workspace.openLinkText(linkPath, "", false);
			return;
		}

		const openInNewLeaf = pane !== MessageRenderer.LINK_PANE_MIDDLE;
		this.app.workspace.openLinkText(linkPath, "", openInNewLeaf);
	}

	/**
	 * Resolve the default pane used when clicking an internal link.
	 *
	 * Reads persisted preference from localStorage and falls back to middle pane.
	 *
	 * @returns Preferred pane identifier
	 */
	private getDefaultLinkPane(): typeof MessageRenderer.LINK_PANE_LEFT | typeof MessageRenderer.LINK_PANE_MIDDLE | typeof MessageRenderer.LINK_PANE_RIGHT {
		try {
			const savedPane = localStorage.getItem(MessageRenderer.LINK_DEFAULT_PANE_STORAGE_KEY);
			if (
				savedPane === MessageRenderer.LINK_PANE_LEFT ||
				savedPane === MessageRenderer.LINK_PANE_MIDDLE ||
				savedPane === MessageRenderer.LINK_PANE_RIGHT
			) {
				return savedPane;
			}
		} catch {
		...
		return MessageRenderer.LINK_PANE_MIDDLE;
	}

	/**
	 * Persist the default pane used for internal-link clicks.
	 *
	 * @param pane - Pane to persist as default
	 */
	private setDefaultLinkPane(
		pane: typeof MessageRenderer.LINK_PANE_LEFT | typeof MessageRenderer.LINK_PANE_MIDDLE | typeof MessageRenderer.LINK_PANE_RIGHT,
	): void {
		try {
			localStorage.setItem(MessageRenderer.LINK_DEFAULT_PANE_STORAGE_KEY, pane);
		} catch {
