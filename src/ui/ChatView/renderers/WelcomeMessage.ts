 ...
 * @param options - Welcome rendering callbacks and preferences
 */
export function renderWelcomeMessage(
	...
	options: {
		...
		onShowWelcomeMessageChange?: (show: boolean) => void;
	}
): void {
	const { onExampleClick, showWelcomeMessage, onShowWelcomeMessageChange } = options;
	...
	learnMoreLink.addEventListener("click", (e) => {
		e.preventDefault();
		window.open("https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent", "_blank");
	});
	...
	const toggleRow = welcomeEl.createDiv({ cls: "vc-welcome-toggle-row" });
	const toggleLabel = toggleRow.createEl("label", { cls: "vc-welcome-toggle-label" });
	const toggleInput = toggleLabel.createEl("input", {
		type: "checkbox",
		cls: "vc-welcome-toggle-input",
	});
	toggleInput.checked = showWelcomeMessage;
	toggleLabel.createSpan({
		text: "Display this welcome message",
		cls: "vc-welcome-toggle-text",
	});
	toggleInput.addEventListener("change", () => {
		onShowWelcomeMessageChange?.(toggleInput.checked);
	});
}
